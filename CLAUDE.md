...
- `data/` : chaîne Wikidata rejouable — `wikidata-fetch.sh` récupère les cinq extraits SPARQL, `wikidata-build.py` en construit `genre-import.csv` (13 familles, 15 tuiles, 1 630 genres). `build-groups.py` écrit `genre-groups.csv`, le découpage éditorial des 36 branches de plus de 8 sous-genres (1 020 genres, colonne `group` du fichier d'import, sous forme de chemin « EUROPE > IBERIA > SPAIN »). Voir `data/README.md`.
